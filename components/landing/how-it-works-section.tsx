...
const steps = [
  {
    number: "01",
    title: "Install",
    description: "One command to install. It pulls in obsidian-mcp (the vault-access server) automatically — no separate install step needed.",
    code: `pip install obsidian-agent-cli-app
...
pipx install obsidian-agent-cli-app`,
  },
  {
    number: "02",
    title: "Configure",
    description: "Run it once. You'll be asked three things — your OpenRouter API key, vault path, and model. Never again.",
    code: `obsidian-agent

# First run prompts:
# → OpenRouter API key
# → Absolute path to your vault
# → Model (defaults to free-tier)

# Saved to ~/.config/obsidian-agent/config.toml`,
  },
  {
    number: "03",
    title: "Chat",
    description: "Start talking to your vault. Ask it to find, summarize, edit, tag, or fix anything. It handles the rest.",
    code: `> What did I write about last week?

Agent: I found 5 notes modified in the
last 7 days. Here's a summary...

> Tag all of those with #weekly-review

Agent: Done. Tagged 5 notes with
#weekly-review.`,
  },
];
    ...
    >
      <div className="max-w-7xl mx-auto px-6 lg:px-8">
        ...
        <div className="mb-20">
          ...
          >
            <span className="text-balance">Three steps to your</span>
            <br />
            <span className="text-balance">AI-powered vault.</span>
          </h2>
        </div>
...
function highlightCode(line: string): string {
  return line
    .replace(/(pip|pipx|obsidian-agent|install)/g, '<span class="text-foreground">$1</span>')
    .replace(/(#.*$)/g, '<span class="text-muted-foreground/50">$1</span>')
    .replace(/(>.*$)/g, '<span class="text-primary">$1</span>')
    .replace(/(Agent:.*$)/g, '<span class="text-green-400">$1</span>');
}
